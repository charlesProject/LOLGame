//初始化分页组件
function initDaLuanDouPagePlugin(qquin,area_id,bt_num,bt_list,champion_id,offset,limit,mvp_flag){
	$.ajax({
		type: "get",
		url: "game/pageData",
		data:{
		...
		success: function(data) {
			...
			$("#pagePluginBox").pagination(data.data[0].total_num, {
		        ...
		        prev_text:"前一页",
		        next_text:"后一页",
		        callback:daLuanDouPaginationClick
			});
		}
	});
...
//mvp按钮监听事件
function bindDaLuanDouBtnEvent(){
		$("#queryDaLuanDou").bind("click",function(){
			var qquin_areaid = $("[data-toggle='modal']").attr("id").split("_");
			var qquin = qquin_areaid[1];
			var area_id = qquin_areaid[0];
			var bt_num = 1;
			...
			var limit = 8;
			var mvp_flag = -1;
			initDaLuanDouPagePlugin(qquin,area_id,bt_num,bt_list,champion_id,offset,limit,mvp_flag);
		})
}
